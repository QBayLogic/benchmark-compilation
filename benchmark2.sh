...
THREAD_STEPS="8 4 2 1"
TIMEF="%C,%x,%e,%U,%S,"
RESULT=$(mktemp --tmpdir="$HOME" benchmark2-results.XXXX.csv)
...
for ghc_threads in ${THREAD_STEPS}; do
       for cabal_threads in ${THREAD_STEPS}; do
               ...
               /usr/bin/time --quiet -p -f ${TIMEF} -o ${RESULT} -a -- cabal new-build clash-ghc --ghc-options="+RTS -qn8 -A32M -RTS -j${ghc_threads}" -j${cabal_threads} || true
               [ $FASTRUN == 1 ] && break
       done
       [ $FASTRUN == 1 ] && break
done
...
for ghc_threads in ${THREAD_STEPS}; do
       for cabal_threads in ${THREAD_STEPS}; do
           rm -rf ~/.cabal/store
           ghc-pkg init ~/.cabal/store/ghc-$GHC_VERSION/package.db
           sed -i "s/^  ghc-options:.*/  ghc-options: +RTS -qn8 -A32M -RTS -j${ghc_threads}/g" ~/.cabal/config
           echo "=========== stack, ghc=${ghc_threads}, cabal=${cabal_threads} ==========="
           /usr/bin/time --quiet -p -f "${TIMEF}GHC_THREADS=${ghc_threads}" -o ${RESULT} -a -- cabal new-install stack-1.9.3 -j${cabal_threads} || true
           [ $FASTRUN == 1 ] && break
       done
       [ $FASTRUN == 1 ] && break
done
